...
Graphiti API 계약을 한곳에 모은 기본 커넥터다. 전사·자료를 에피소드로 변환하고 50,000자
상한을 처리하며, 그래프 조회·질문·상세 조회를 같은 Graphiti 프로젝트로 전달한다.

## 환경변수

- Supabase Auth: `SUPABASE_URL`, `SUPABASE_ANON_KEY`
- Graphiti: 선택 `GSVX_BASE_URL`, `GSVX_API_KEY` (미지정 시 배포된 Graphiti 서비스)
